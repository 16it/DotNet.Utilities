namespace YanZhiwei.DotNet.KafkaLog4Net
{
    using System;
    using System.IO;
    using System.Text;
    ...
    public class KafkaAppender : AppenderSkeleton
    {
        ...
        protected override void Append(LoggingEvent loggingEvent)
        {
            try
            {
                var _message = GetLogMessage(loggingEvent);
                var _topic = GetTopic(loggingEvent);

                kafkaProducer.SendMessageAsync(_topic, new[] { new Message(_message) });
            }
            catch (Exception ex)
            {
        ...
        private string GetLogMessage(LoggingEvent loggingEvent)
        {
            ...
            using (StringWriter writer = new StringWriter(_builder))
            {
