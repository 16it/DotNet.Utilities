...
namespace YanZhiwei.DotNet.Dapper.UtilitiesTests
{
    ...
    public class DapperSqlServerOperatorTests
    {
        ...
        public void ExecuteNonQueryTest()
        {
            ...
            DataTable _table = sqlHelper.ExecuteDataTable<Person>("SELECT [username] FROM [Person] where username=@userName", new Person()
            {
                username = "YanZhiwei"
            });
            Assert.AreEqual(_table.Rows[0][0], "YanZhiwei");
            sqlHelper.ExecuteNonQuery<Person>("DELETE FROM [Person] where username=@username", new Person()
            {
                username = "YanZhiwei"
            });
        }
        ...
        {
            using(IDataReader reader = sqlHelper.ExecuteReader("SELECT  [id],[username],[password],[age],[registerDate],[address] FROM [Person] where id=@id", new Person()
            {
                id = 5
            }))
            {
        ...
        {
            Person _person = sqlHelper.Query<Person>("SELECT  [id],[username],[password],[age],[registerDate],[address] FROM [Person] where id=@id", new Person()
            {
                id = 5
            });
        ...
        {
            var _userName = sqlHelper.ExecuteScalar("SELECT [username] FROM [Person] where id=@id", new Person()
            {
                id = 5
            });
